...
export function showToast(message, type = 'success') {
  ...
  toastElement.classList.remove('bg-success', 'bg-danger', 'bg-info'); // Remove any previous types
  if (type === 'error') {
...
export function showError(message, duration = 3000) {
  ...
  errorDiv.classList.add('error', 'alert', 'alert-danger'); // Default 'danger' style
